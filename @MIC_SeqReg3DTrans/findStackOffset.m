...
%
% INPUTS:
%   Stack1: (mxnxo) The stack to which Stack2 is compared to, i.e. Stack1 
%           is the reference stack.
%   Stack2: (pxqxr) The stack for which the offset relative to Stack1 is
%           to be determined.
...
%               i.e. mean subtracted and scaled by their auto-correlation) 
%               to compute a xcorr coefficient field which is then 
%               appropriately scaled by the xcorr coefficient field of two
%               binary stacks of size(Stack1), size(Stack2), respectively.
...
%               coefficient field.
%   FitType: (string/character array)(default = '3DLineFits') The type of
...
%            '3D' fits a 2nd order polynomial (with cross terms) to the 3D
%               xcorr coefficient field using the least-squares method.
%            '3DLineFits' fits a 2nd order polynomial (w/o cross terms) to
%               three lines parallel to x, y, and z which intersect the 
...
switch FitType
    ...
    case '3DLineFits'
        ...
        RepArrayZ = [ones(numel(XArray) + numel(ZArray), 1) ...
            * RawOffsetIndices(3); ...
