...
void mexFunction(int nlhs, mxArray *plhs[],	int	nrhs, const	mxArray	*prhs[]) {
	...
	// short __cdecl KPC_Open  ( char const *  serialNo )

	if (nrhs != 1)
		mexErrMsgTxt("Proper Usage: Err=Kinesis_KPC_Open('SerialNoString')");

	if (!mxIsClass(prhs[0], "char"))
		mexErrMsgTxt("Proper Usage: Err=Kinesis_KPC_Open('SerialNoString').  Input must be character array.");
	...
	short Err = KPC_Open(input_buf);
	mexPrintf("Opened %s\n", input_buf);
	plhs[0] = mxCreateDoubleScalar(Err);
	if (!Err) { //If errror opening, send back error code.
		KPC_LoadSettings(input_buf); //load actuator settings (max travel etc.) into DLL
		KPC_Identify(input_buf);
		KPC_StartPolling(input_buf, 200);
	}
	mxFree(input_buf);
	return;
 }
