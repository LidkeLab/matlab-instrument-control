...
StackSize = [size(RefStack, 1), size(RefStack, 2), size(RefStack, 3)];
...
while (any(abs(NewShift)>Tolerance) && (ii<NIterMax))
    ...
    % cross-correlation).
    StackHalfWidth = floor([size(MovingStack, 1), ...
        size(MovingStack, 2), size(MovingStack, 3)] / 2);
    for nn = 1:3
        if (NewShift(nn) < 0)
            BorderDirection = 'pre';
        else
            BorderDirection = 'post';
        end
        Border = [0, 0, 0];
