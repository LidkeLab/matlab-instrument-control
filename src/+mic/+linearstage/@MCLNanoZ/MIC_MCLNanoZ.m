classdef MCLNanoZ < mic.linearstage.abstract
    % mic.linearstage.MCLNanoZ controls the Mad City Labs 3D Piezo Stage
    %
    %   ## Description
    ...
    %
    % ## Key Methods
    % - **Constructor (`mic.linearstage.MCLNanoZ()`):** Initializes the connection to the Mad City Labs stage and loads necessary libraries.
    ...
    % ## Usage Example
    % ```matlab
    % % Create an instance of the MCLNanoZ
    % stage = mic.linearstage.MCLNanoZ();
    %
    ...
    % ### Citation:Hanieh Mazloom-Farsibaf, Lidke Lab 2018
     ...
     properties (SetAccess=protected)
        ...
        CurrentPosition=0;    %Current Position (micron)
        %SensorPosition;      %Results of a get Position (micron)
        PositionUnit='um'    %Units of position parameter (micron)
        Axis = 'Z';
        WaitTime=0.05;                 %Time to wait before returning after a setPosition (s)
    end
    
    % properties(Transient, SetAccess = protected)
    %    Position=0;    %Current Position (micron)
    %    SensorPosition;      %Results of a get Position (micron)
    %    PositionUnit='um'    %Units of position parameter (micron)
    % end
    ...
    methods
        ...
        function obj=MCLNanoZ()
            ...
            [p,~]=fileparts(which('MIC_MCLNanoZ'));
            if exist(fullfile(p,'MIC_MCLNanoZ_Properties.mat'),'file')
                a=load(fullfile(p,'MIC_MCLNanoZ_Properties.mat'));
            ...
            obj.MaxPosition = obj.errorcheck('MCL_GetCalibration');
        ...
        function getdllpath(obj)
            ...
            [p,~]=fileparts(which('mic.linearstage.MCLNanoZ'));
            f=fullfile(p,'MIC_MCLNanoZ_Properties.mat');
            DLLPath=obj.DLLPath;
            save(f,'DLLPath');
        end
        ...
        function setPosition(obj,Position)            
            ...
            % Z
            %if z < 0 || z > obj.Max_Z
            %    error('MCLNanoZ:InvalidZ','Z position must be between 0 and %f�m.', obj.Max_Z);
            %end
            obj.CurrentPosition=max(obj.MinPosition,Position);
            obj.CurrentPosition=min(obj.MaxPosition,obj.CurrentPosition);
            obj.LastError = obj.callNano('MCL_SingleWriteN',obj.CurrentPosition,3,obj.handle);
            obj.errorcheck('MCL_SingleWriteN',obj.CurrentPosition);            
            %This updates the gui if it exists
            % h = findall(0,'tag','MIC_MCLNanoZ_gui');
            % if ~(isempty(h))
            %     handles=guidata(h);
            %     X=obj.Position;
    ...
    methods (Static)
        ...
        function Success=unitTest()
            ...
            try
                ...
                M=mic.linearstage.MCLNanoZ()
                fprintf('Setting Position to 10\n')
