classdef simulated_LinearStage < mic.linearstage.abstract
    ...
    %
    % ### `exportState()`
    % - **Description:** Exports relevant properties for saving the state of the instrument.
    %   - **Attributes:** Includes properties like `PositionUnit`, `CurrentPosition`, `MinPosition`, `MaxPosition`, and `Axis`.
    %   - **Data:** No specific data exported.
    %   - **Children:** No children components.
    %
    % ### Static Method: `funcTest()`
    % - **Description:** Tests the functionality of the `simulated_LinearStage` class.
    %   - Creates an instance, moves it to a position, and tests for basic functionality.
